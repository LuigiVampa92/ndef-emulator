...
Эта библиотека эмулирует метку "NFC Forum Type 4 Tag" содержащую NDEF-форматированное сообщение.
...
Эти тэги нативно поддерживаются в большинстве ПО которое умеет читать NDEF-форматированные метки по NFC. Также их чтение нативно поддерживают обе мобильные операционные системы, и Android, и iOS. 
...
Этот класс содержит свойство `currentEmulatedNdefData` (или геттер и сеттер `getCurrentEmulatedNdefData()`/`setCurrentEmulatedNdefData()` если вы используете Java) которое используется для управления данными, которые необходимо эмулировать.
